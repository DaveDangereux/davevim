...
local on_attach = function(server, bufnr)
  ...
  if server.server_capabilities.documentSymbolProvider then
    require("nvim-navic").attach(server, bufnr)
  end
...
local on_server_ready = function(server)
  ...
  if server.name == "jsonls" then
    local jsonls_opts = require("config.lsp.configs.jsonls")
    opts = vim.tbl_deep_extend("force", jsonls_opts, opts)
  end

  if server.name == "sumneko_lua" then
    local sumneko_opts = require("config.lsp.configs.sumneko_lua")
    opts = vim.tbl_deep_extend("force", sumneko_opts, opts)
  end

  if server.name == "emmet_ls" then
    local emmet_ls_opts = require("config.lsp.configs.emmet_ls")
